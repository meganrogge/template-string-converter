    ...
    if (enabled && changes) {
      try {
        ...
        let openingQuotePosition = new vscode.Position(
          lineNumber,
          getStartQuote(lineText, getQuoteChar(quoteType))
        );
        let endQuotePosition = new vscode.Position(
          lineNumber,
          getEndQuote(lineText, getQuoteChar(quoteType))
        );
